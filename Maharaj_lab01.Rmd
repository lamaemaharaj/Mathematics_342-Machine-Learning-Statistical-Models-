...
```{r}
# The sum of each row should be either 0 or 1. Due to the fact that these events appear to be mutually exclusive.
...
k=100
X = matrix(data=NA,nrow=k,ncol=6)
X[,1]= rnorm(n=k,mean = 17,sd=sqrt(38))
X[,2]= runif(n=k,min=-10,max=10)
X[,3]= rpois(k,lambda=6)
X[,4]= rexp(k,rate = 9)
X[,5]= rbinom(k, size=20,p=0.12)
X[,6]= sample(c(rep(1,k*0.24),rep(0,k*0.76)))
...
```{r}
n = 100
R = matrix(rnorm(n^2), nrow = n, ncol = n)
for (i in 1 : n){
  for (j in 1 : n){
    if (runif(1) < 0.3){
      R[i,j] = NA
...
```{r}
v <-apply(X=R[,],MARGIN = 2,FUN = sd,na.rm=TRUE)
v
```
